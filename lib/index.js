...
function S3(uri, callback) {
    ...
    if (uri.host) uri = 'http://' + uri.host + '.s3.amazonaws.com' + uri.pathname;
...
S3.prototype.startWriting = function(callback) {
    ...
    this.uri = this._uri || url.parse(this.data.tiles[0]);
    this.putPath = this.uri.pathname;
    if (this.data.grids) {
        this.griduri = url.parse(this.data.grids[0]);
        this.gridPutPath = this.griduri.pathname;
    }
